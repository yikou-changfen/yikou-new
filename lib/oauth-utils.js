...
function verifyState(request, response, provider, state) {
  ...
  if (!state || !expected || state !== expected) return { ok: false, code: "OAUTH_STATE_MISMATCH" };
  const [payload, signature] = state.split(".");
  if (!payload || !signature || signState(payload) !== signature) return { ok: false, code: "OAUTH_STATE_INVALID" };
  try {
...
module.exports = { clearStateCookie, createSignedState, getBaseUrl, missingOAuth, setStateCookie, verifyState };
